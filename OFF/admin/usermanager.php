...
if (!empty($_GET['steam_id'])) {
    ...
    $req = $q->fetch();
    if ($req['count'] == 1) {
        //Get user info in database for the next commands
        ...
        //Reget all user info after modifications are done
        $q = $db->prepare('SELECT * from users WHERE steam_id=?');
        $q->execute([$_GET['steam_id']]);
        $userInfo = $q->fetch();
    }
                ...
                <div class="content-box-large box-with-header">
                    <?php if (!isset($userInfo)) { ?>
                        <form method="get" action="usermanager.php">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="steam_id" aria-describedby="steam_id"
                                       name="steam_id" placeholder="Enter steam id">

                            </div>
                        </form>
                        <h3>All Users</h3>
                        <small>Rank (1 = Admin, 0 = User)</small><br>
                        <small>Rank (1 = Banned, 0 = Not Banned)</small><br>
                    ...
                    <?php } else { ?>
                        <div class="jumbotron" align="center">
                            <img class="glow2 img-rounded" src="<?php echo $userInfo['avatar_medium'] ?>" alt=""
